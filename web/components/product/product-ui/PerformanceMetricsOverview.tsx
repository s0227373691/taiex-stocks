    ...
    return (
        <div className="p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800">
            <dl className="grid max-w-screen-xl grid-cols-2 gap-8 p-4 mx-auto text-gray-900 sm:grid-cols-3 xl:grid-cols-6 dark:text-white sm:p-8">
                <StatItem label="ATH Price" value={`${ath}`} />
                <StatItem label="Max Drawdown Price" value={`${maxDrawdown}`} />
                <StatItem label="Open source projects" value="1000s" />
                <StatItem label="ATH Ratio" value={`${athRatio}%`} />
                <StatItem
                    label="Distance to maxDrawdown ratio"
                    value={`${maxDrawdownRatio}%`}
                />
