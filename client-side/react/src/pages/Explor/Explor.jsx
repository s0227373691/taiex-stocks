...
const Explor = () => {
    const { isLoading, isError, error, isSuccess, data: snapshotDatas } = useQuery('fetchSnapshot', () => fetchSnapshot('TSE,OTC'))
    const [snapshotFilteredDatas, setSnapshotFilteredDatas] = useState([])
    ...
    useEffect(() => {
        if (snapshotDatas) {
            ...
            snapshotDatas.map(snapshot => {
                const newSnapshotData = snapshot.data.filter(el => el.symbol.length === 4)
