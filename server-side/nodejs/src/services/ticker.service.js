...
async function syncExternalData() {
    try {
        ...
        const equityPromises = equitys?.tikers.map((equity) => updateTicker(equity))
...
async function updateTicker(_equity) {
    return TickersModel.findOneAndUpdate(
        ...
        { symbol: _equity.symbol, name: _equity.name, type: _equity.type, updateTime: new Date(_equity.date) },
        {
            new: true,
            upsert: true,
        }
